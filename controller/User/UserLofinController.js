...
exports.insertAmount = async (req, res) => {
  try {
    ...
    if (!amount || isNaN(amount) || !userId) {
      return res
        .status(400)
        .json({ message: "Valid amount and userId are required." });
    }
    ...
    // Find the user in the FrontRegistrationData collection
    const userRecord = await FrontRegistrationData.findOne({ _id: userId });
    if (!userRecord) {
      return res.status(404).json({ message: "User ID not found." });
    }
    // console.log(userRecord, "userRecord");
    // Add the new amount to the existing amount
    userRecord.amount += parsedAmount;
...
exports.fetchCompanyName = async (req, res) => {
  try {
...
exports.registerfrontendData = async (req, res) => {
  ...
  try {
    // Check if user with the same email already exists
    const existingUser = await FrontRegistrationData.findOne({ email });

    if (existingUser) {
      return res.status(400).json({
        message: "Email already exists. Please provide unique information."
      });
    }
...
exports.userDetailDataReg = async (req, res) => {
  try {
    ...
    const reversedUsers = await FrontRegistrationData.find();
    const users = reversedUsers.reverse();
    res.status(200).json({
...
exports.deleteUserdata = async (req, res) => {
  try {
    ...
    const deletedUser = await FrontRegistrationData.findByIdAndDelete(id);
